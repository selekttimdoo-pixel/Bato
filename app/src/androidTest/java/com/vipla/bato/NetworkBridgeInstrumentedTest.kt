...
class NetworkBridgeInstrumentedTest {
    ...
    fun installedManifestAndLiveProviderPostWorkOnAndroid() = runBlocking {
        ...
        val bridge = HttpProviderBridge("https://bato-sigma.vercel.app/api/bato")

        val result = bridge.respond("Odgovori samo: ANDROID BATO LIVE OK", ProviderContext(emptyList(), emptyList(), emptyList(), emptyList()))
        assertTrue("Expected a real provider response, got $result", result is ProviderResult.Response)
        assertTrue("Unexpected provider text: $result", (result as ProviderResult.Response).text.contains("ANDROID BATO LIVE OK", ignoreCase = true))

        val date = bridge.respond("Koji je danas dan? U odgovoru obavezno navedi datum kao YYYY-MM-DD.", ProviderContext(emptyList(), emptyList(), emptyList(), emptyList()))
        ...
        val memory = bridge.respond("Koje je moje test ime?", ProviderContext(listOf(prior), emptyList(), emptyList(), emptyList()))
        ...
        val retrieval = bridge.respond("Koji je kod trezora?", ProviderContext(emptyList(), listOf(stored), emptyList(), emptyList()))
